...
<div class="modal-body">
    <form class="mt-2" role="form" id="updateDev"  enctype="multipart/form-data" data-parsley-validate="">
        {{ csrf_field() }}
        <input type="hidden" id="fid"  name="preliminary_form_interview" value="{{$applicant->id}}">
        <h4 class="mb-8">{{ $applicant->first_name }} {{ $applicant->last_name }}</h4>
        <div class="row mt-md-5 mb-4">
            <div class="col-md-6 col-sm-12 align-self-center">
                <label for="email_edit" class="label-sm hr-default-text"> Actual Company *</label>
                <input type="text" class="form-control required info-required" name="user[actual_company]"
            ...
            <div class="col-md-6 col-sm-12 align-self-center">
                ...
                <input type="text" class="form-control required info-required" name="user[actual_position]"
        ...
        @php
            $categoryNames = array('Front End' => 'Front-End Developer',
              'Back End' => 'Back-End Developer',
              'Database' => 'Database',
              'Mobile' => 'Mobile',
              'Other Developer' => 'Other (Specify)');
        ...
        <div class="row mt-md-3 mb-4 mx-2">
            ...
            <div class="list-container">
                @foreach ($skillCategories as $cat)
                    ...
                    @endphp
                    <div class="technology <?php if($cat->mainCategory == 'Other Developer') { echo 'other-techno'; } ?>">
                        @if($cat->mainCategory == 'Other Developer')
                            <div class="col-md-12">
        ...
        <div class="col-12 mt-md-5 mb-4 pl-0">
            <h5 class="dev-form-header mt-0">Professional Self-Evaluation (max 3-4 sentences)</h5>
            <textarea name="user[professional_self_evaluation]"
                      value="{{old('user.professional_self_evaluation')? old('user.professional_self_evaluation') : $applicant->professional_self_evaluation}}"
                      id="professional_self_evaluation_edit" rows="10">{{$applicant->professional_self_evaluation}}</textarea>
        </div>

        <div class="col-12 modal-footer">
            <button type="button" class="btn btn-small btn-secondary" data-dismiss="modal"><i class="fa fa-ban"></i>&nbsp;Close</button>
            <button type="button" class="btn btn-small btn-success saveDev"><i class="fa fa-floppy-o"></i>&nbsp;Save</button>
        </div>
    </form>
</div>
<script>
    ...
    function saveDevData() {
        var data = $("#updateDev").serializeArray();
    ...
    $(document).ready(function () {
        ...
        $(".saveDev").on('click', function(){
            // saveData();
            $(this).parsley().validate();
            if($('#updateDev').parsley().isValid()) {
                saveDevData();
            } else {
                setTimeout(function () {
                    $('#updateDev').parsley().validate();
                }, 50);
            }
        });
